  ...
  <div class="scratch-con">
    ...
    <div class="main-box" v-show="isOk">
      ...
      <div class="sub-box">
        ...
        <div style="user-focus: none; user-select: none" class="success-text">
          恭喜您成功刮出小鸡，继续下一步操作吧
        </div>
      </div>
    </div>
    <canvas class="canvas" v-show="!isOk" id="canvas"
      >请换一个现代浏览器</canvas
    >
  </div>
</template>
          ...
          if (count === 2) {
            ...
            function bindTouchEvent() {
              ...
              canvas.addEventListener("touchmove", (e) => {
                ...
                for (let i = 0; i < tapTail.length; i++) {
                  // handle repeat
                ...
                tapTail.push(
                  new Tap(offsetX, offsetY, {
                    r: tapRadius,
                    virtualCanvas: virtual,
                    ctx,
                  })
                );
                for (let i = 0; i < chickenList.length; i++) {
                  ...
                  if (c.activeCount >= c.points.length) {
                    // all active
                    c.finish = true;
                  }
                }
            ...
            function loop() {
              ...
              if (
                ...
                new Date().getTime() - startTimestamp >= 1000 * duration
              ) {
                // 超出限时还没刮完
            ...
            function drawBase() {
              ...
              while (chickenList.length < targetCount) {
                const c = new Chicken(
                  random(cWidth, width - cWidth),
                  random(cHeight, height - cHeight),
                  {
                    width: cWidth,
                    height: cHeight,
